<template>
    <div class="report_count-container">
      ...
      <div class="content-box" id="reportContent">
        <h1>Farmer Report Search by Emergency Contact</h1>
        <div>
          <div class="input">
            <label for="personToNotify">Select Person(s) to Notify in Case of Emergency:</label>
            <div class="selected-contacts-container">
              <div class="selected-contacts">
                <!-- Display selected emergency contacts as tags -->
                <span v-for="contact in selectedContacts" :key="contact" class="tag">
                  {{ contact }}
                  <button @click="removeContact(contact)">x</button>
                </span>
              </div>
              <!-- Download Button -->
            ...
            <select v-model="selectedContact" @change="addContact" required spellcheck="false" class="black-select">
              <option disabled value="">Select Person to Notify</option>
              <option v-for="contact in emergencyContacts" :key="contact" :value="contact">
                {{ contact }}
              </option>
            </select>
          </div>
        ...
        <div v-if="selectedContacts.length > 0 && farmers.length > 0" class="total-count">
        <h2>Total Farmers: {{ farmers.length }}</h2>
      </div>
  
        <!-- Display farmers for the selected emergency contacts -->
        <div v-if="farmers.length > 0">
          <h2>Farmers to Notify in Case of Emergency: {{ selectedContacts.join(', ') }}:</h2>
        ...
        <div v-else-if="selectedContacts.length > 0">
          <p>No farmers found with selected emergency contacts: {{ selectedContacts.join(', ') }}.</p>
        </div>
      </div>
    </div>
  </template>
  ...
  const fetchEmergencyContacts = async () => {
    try {
      const response = await axios.get(
        'http://localhost:8055/items/farmers?fields=person_to_notify_in_case_of_emergency',
        {
          headers: {
      ...
      const contactList = response.data.data.map(farmer => farmer.person_to_notify_in_case_of_emergency);
      emergencyContacts.value = [...new Set(contactList)];
    } catch (error) {
  ...
  const addContact = () => {
    if (selectedContact.value && !selectedContacts.value.includes(selectedContact.value)) {
      selectedContacts.value.push(selectedContact.value);
      selectedContact.value = '';
      fetchFarmersByContact();
    }
  };
  ...
  const fetchFarmersByContact = async () => {
    farmers.value = []; // Clear previous data to reorder based on selection
    try {
      if (selectedContacts.value.length > 0) {
        for (const contact of selectedContacts.value) {
          const response = await axios.get(
            `http://localhost:8055/items/farmers?filter[person_to_notify_in_case_of_emergency]=${contact}`,
            {
              headers: {
                Authorization: `Bearer ${token}`, 
              },
  ...
  const downloadPDF = async () => {
    ...
    for (let i = 0; i < farmers.value.length; i += maxRowsPerPage) {
      ...
      pageContent.innerHTML = `
        <h1 style="color: black;">Farmer Report by Emergency Contact - Page ${(i / maxRowsPerPage) + 1}</h1>
        ...
        <thead class="table-primary" style="background-color: #007bff; color: black;">
          <tr>
            ...
            <th style="padding: 0.75rem; text-align: left; border: 1px solid black; color: white;">Emergency Contact</th>
          </tr>
        </thead>
        <tbody>
          ${farmersPage.map(farmer => `
            <tr>
              ...
              <td style="padding: 0.75rem; text-align: left; border: 1px solid black; color: black;">${farmer.person_to_notify_in_case_of_emergency}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    const options = {
      ...
      filename: 'farmer_report_by_emergency_contact.pdf',
