<template>
  <div class="report_count-container">
    ...
    <div class="content-box" id="reportContent">
      ...
      <!-- Display grouped farmers by farming activity -->
      <div v-if="groupedFarmers.length > 0">
        ...
        <div class="table-responsive">
          <table class="table">
            ...
            <tbody>
              <tr v-for="(activity, index) in groupedFarmers" :key="activity.id">
                <td>{{ index + 1 }}</td>
                <td>{{ activity.farming_activity }}</td>
...
const groupedFarmers = ref([]); // To hold grouped farmers by farming activity
const token = localStorage.getItem('auth_token');

// Fetch and group farmers by farming activity
const fetchFarmers = async () => {
  try {
    const response = await axios.get(
      'http://localhost:8055/items/farmers?fields=type_of_farming_activity', // Change to `type_of_farming_activity`
      {
        headers: {
    ...
    // Group farmers by their type of farming activity (which is an array)
    const activityCounts = {};

    response.data.data.forEach(farmer => {
      const activities = farmer.type_of_farming_activity || [];
    ...
    groupedFarmers.value = Object.entries(activityCounts).map(([farming_activity, count], index) => ({
      ...
      <table class="table" style="border-collapse: collapse; width: 100%;">
        ...
        <tbody>
          ${farmersPage.map(farmer => `
            <tr>
              <td style="padding: 0.75rem; text-align: left; border: 1px solid black; color: black;">${farmer.farming_activity}</td>
