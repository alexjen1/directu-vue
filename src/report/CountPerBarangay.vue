<template>
  <div class="report_count-container">
    ...
    <div class="content-box" id="reportContent">
      ...
      <!-- Display grouped farmers by barangay -->
...
const groupedFarmers = ref([]); // To hold grouped farmers by barangay
const token = localStorage.getItem('auth_token');

// Fetch and group farmers by barangay
const fetchFarmers = async () => {
  try {
    const response = await axios.get(
      'http://localhost:8055/items/farmers?fields=barangay', // Change `civil_status` to `barangay`
      {
        headers: {
    ...
    groupedFarmers.value = Object.entries(barangayCounts).map(([barangay, count], index) => ({
