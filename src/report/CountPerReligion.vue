<template>
    <div class="report_count-container">
      ...
      <div class="content-box" id="reportContent">
        <h1>Farmer Report Search by Religion</h1>
        <div>
          <div class="input">
            <label for="religion">Select Religion(s):</label>
            <div class="selected-religions-container">
              <div class="selected-religions">
                <!-- Display selected religions as tags -->
                <span v-for="religion in selectedReligions" :key="religion" class="tag">
                  {{ religion }}
                  <button @click="removeReligion(religion)">x</button>
                </span>
              </div>
              <!-- Download Button -->
            ...
            <select v-model="selectedReligion" @change="addReligion" required spellcheck="false" class="black-select">
        ...
        <div v-if="selectedReligions.length > 0 && farmers.length > 0" class="total-count">
        <h2>Total Farmers: {{ farmers.length }}</h2>
      </div>
  
        <!-- Display farmers for the selected religions -->
        <div v-if="farmers.length > 0">
          <h2>Farmers with Religion {{ selectedReligions.join(', ') }}:</h2>
        ...
        <div v-else-if="selectedReligions.length > 0">
          <p>No farmers found with religion {{ selectedReligions.join(', ') }}.</p>
        </div>
      </div>
    </div>
  </template>
  ...
  const fetchReligions = async () => {
    try {
      const response = await axios.get(
        'http://localhost:8055/items/farmers?fields=religion',
        {
          headers: {
      ...
      const religionList = response.data.data.map(farmer => farmer.religion);
      religions.value = [...new Set(religionList)];
    } catch (error) {
  ...
  const addReligion = () => {
    if (selectedReligion.value && !selectedReligions.value.includes(selectedReligion.value)) {
      selectedReligions.value.push(selectedReligion.value);
      selectedReligion.value = '';
      fetchFarmersByReligion();
    }
  };
  
  const removeReligion = (religion) => {
  ...
  const fetchFarmersByReligion = async () => {
    farmers.value = []; // Clear previous data to reorder based on selection
    try {
      if (selectedReligions.value.length > 0) {
        for (const religion of selectedReligions.value) {
          const response = await axios.get(
            `http://localhost:8055/items/farmers?filter[religion]=${religion}`,
            {
              headers: {
                Authorization: `Bearer ${token}`, 
              },
  ...
  const downloadPDF = async () => {
      ...
      pageContent.innerHTML = `
        ...
        <thead class="table-primary" style="background-color: #007bff; color: black;">
          <tr>
            ...
            <th style="padding: 0.75rem; text-align: left; border: 1px solid black; color: white;">Religion</th>
          </tr>
        </thead>
        <tbody>
    ...
    const options = {
      ...
      filename: 'farmer_report_religion.pdf',
