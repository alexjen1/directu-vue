<template>
    <div class="report_count-container">
      ...
      <div class="content-box" id="reportContent">
        <h1>Farmer Report Search by Street/Sitio/Subdivision</h1>
        <div>
          <div class="input">
            <label for="street_sitio_subdv">Select Street/Sitio/Subdivision(s):</label>
            <div class="selected-streets-container">
              <div class="selected-streets">
                <!-- Display selected streets as tags -->
                <span v-for="street in selectedStreets" :key="street" class="tag">
                  {{ street }}
                  <button @click="removeStreet(street)">x</button>
                </span>
              </div>
              <!-- Download Button -->
            ...
            <select v-model="selectedStreet" @change="addStreet" required spellcheck="false" class="black-select">
              <option disabled value="">Select Street/Sitio/Subdivision</option>
        ...
        <div v-if="selectedStreets.length > 0 && farmers.length > 0" class="total-count">
        <h2>Total Farmers: {{ farmers.length }}</h2>
      </div>

  
        <!-- Display farmers for the selected streets -->
        <div v-if="farmers.length > 0">
          <h2>Farmers in {{ selectedStreets.join(', ') }}:</h2>
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="farmer in farmers" :key="farmer.id">
                  ...
                  <td>{{ farmer.street_sitio_subdv }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div v-else-if="selectedStreets.length > 0">
          <p>No farmers found in {{ selectedStreets.join(', ') }}.</p>
        </div>
      </div>
    </div>
  </template>
  ...
  const fetchStreets = async () => {
    try {
      const response = await axios.get(
        'http://localhost:8055/items/farmers?fields=street_sitio_subdv',
        {
          headers: {
      ...
      const streetList = response.data.data.map(farmer => farmer.street_sitio_subdv);
      streets.value = [...new Set(streetList)];
    } catch (error) {
  ...
  const addStreet = () => {
    if (selectedStreet.value && !selectedStreets.value.includes(selectedStreet.value)) {
      selectedStreets.value.push(selectedStreet.value);
      selectedStreet.value = '';
      fetchFarmersByStreet();
    }
  };
  ...
  const fetchFarmersByStreet = async () => {
    farmers.value = [];
    try {
      if (selectedStreets.value.length > 0) {
        for (const street of selectedStreets.value) {
          const response = await axios.get(
            `http://localhost:8055/items/farmers?filter[street_sitio_subdv]=${street}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
...
const downloadPDF = async () => {
    ...
    for (let i = 0; i < farmers.value.length; i += maxRowsPerPage) {
      ...
      pageContent.innerHTML = `
        <h1 style="color: black;">Farmer Report by Street Sitio Subdv - Page ${(i / maxRowsPerPage) + 1}</h1>
        ...
        <thead class="table-primary" style="background-color: #007bff; color: black;">
          <tr>
            ...
            <th style="padding: 0.75rem; text-align: left; border: 1px solid black; color: white;">Street Sitio Subdv</th>
          </tr>
        </thead>
        <tbody>
          ${farmersPage.map(farmer => `
            <tr>
              ...
              <td style="padding: 0.75rem; text-align: left; border: 1px solid black; color: black;">${farmer.street_sitio_subdv}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    const options = {
      ...
      filename: 'farmer_report_sitio.pdf',
