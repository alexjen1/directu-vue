<template>
    <div class="report_count-container">
      ...
      <div class="content-box" id="reportContent">
        ...
        <div>
          <div class="input">
            <label for="indigenousGroup">Select Indigenous Group(s):</label>
            <div class="selected-indigenous-group-container">
              <div class="selected-indigenous-groups">
                <!-- Display selected indigenous groups as tags -->
                <span v-for="group in selectedIndigenousGroups" :key="group" class="tag">
                  {{ group }}
                  <button @click="removeIndigenousGroup(group)">x</button>
                </span>
              </div>
              <!-- Download Button -->
            ...
            <select v-model="selectedIndigenousGroup" @change="addIndigenousGroup" required spellcheck="false" class="black-select">
              <option disabled value="">Select Indigenous Group</option>
              <option v-for="group in indigenousGroups" :key="group" :value="group">
                {{ group }}
              </option>
            </select>
          </div>
        </div>
        <div v-if="selectedIndigenousGroups.length > 0 && farmers.length > 0" class="total-count">
        <h2>Total Farmers: {{ farmers.length }}</h2>
      </div>
  
        <!-- Display farmers for the selected indigenous groups -->
        <div v-if="farmers.length > 0">
          <h2>Farmers in {{ selectedIndigenousGroups.join(', ') }}:</h2>
        ...
        <div v-else-if="selectedIndigenousGroups.length > 0">
          <p>No farmers found in {{ selectedIndigenousGroups.join(', ') }}.</p>
        </div>
      </div>
    </div>
  </template>
...
const selectedIndigenousGroups = ref([]);
const selectedIndigenousGroup = ref('');
const indigenousGroups = ref([]);
const farmers = ref([]);

const token = localStorage.getItem('auth_token');

// Fetch unique indigenous groups
const fetchIndigenousGroups = async () => {
  try {
    ...
    const groupList = response.data.data.map(farmer => farmer.member_of_an_indiginous_group);
    indigenousGroups.value = [...new Set(groupList)];
  } catch (error) {
    console.error('Error fetching indigenous groups:', error);
  }
};

// Add selected indigenous group
const addIndigenousGroup = () => {
  if (selectedIndigenousGroup.value && !selectedIndigenousGroups.value.includes(selectedIndigenousGroup.value)) {
    selectedIndigenousGroups.value.push(selectedIndigenousGroup.value);
    selectedIndigenousGroup.value = '';
    fetchFarmersByIndigenousGroup();
  }
};

// Remove indigenous group
const removeIndigenousGroup = (group) => {
  const index = selectedIndigenousGroups.value.indexOf(group);
  if (index > -1) {
    selectedIndigenousGroups.value.splice(index, 1);
    fetchFarmersByIndigenousGroup();
  }
};

// Fetch farmers by indigenous group
const fetchFarmersByIndigenousGroup = async () => {
  farmers.value = []; // Clear previous data to reorder based on selection
  try {
    if (selectedIndigenousGroups.value.length > 0) {
      for (const group of selectedIndigenousGroups.value) {
        const response = await axios.get(
          `http://localhost:8055/items/farmers?filter[member_of_an_indiginous_group]=${group}`,
          {
            headers: {
              Authorization: `Bearer ${token}`, 
            },
...
const downloadPDF = async () => {
  ...
  for (let i = 0; i < farmers.value.length; i += maxRowsPerPage) {
    ...
    pageContent.innerHTML = `
      <h1 style="color: black;">Farmer Report by Indigenous Group - Page ${(i / maxRowsPerPage) + 1}</h1>
      <table class="table" style="border-collapse: collapse; width: 100%;">
        <thead class="table-primary" style="background-color: #007bff; color: black;">
          <tr>
            ...
            <th style="padding: 0.75rem; text-align: left; border: 1px solid black; color: white;">Indigenous Group</th>
          </tr>
        </thead>
        <tbody>
          ${farmersPage.map(farmer => `
            <tr>
              ...
              <td style="padding: 0.75rem; text-align: left; border: 1px solid black; color: black;">${farmer.member_of_an_indiginous_group}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  const options = {
    ...
    filename: 'farmer_report_indi_group.pdf',
...
.selected-indigenous-group-container {
  align-items: center;
  justify-content: space-between; /* Adjusts space between elements */
}

.selected-indigenous-group {
  display: flex;
