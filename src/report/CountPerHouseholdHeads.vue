<template>
    <div class="report_count-container">
      ...
      <div class="content-box" id="reportContent">
        <h1>Farmer Report Search by Household Head</h1>
        <div>
          <div class="input">
            <label for="householdHead">Select Household Head(s):</label>
            <div class="selected-household-heads-container">
              <div class="selected-household-heads">
                <!-- Display selected household heads as tags -->
                <span v-for="householdHead in selectedHouseholdHeads" :key="householdHead" class="tag">
                  {{ householdHead }}
                  <button @click="removeHouseholdHead(householdHead)">x</button>
                </span>
              </div>
              <!-- Download Button -->
            ...
            <select v-model="selectedHouseholdHead" @change="addHouseholdHead" required spellcheck="false" class="black-select">
              <option disabled value="">Select Household Head</option>
              <option v-for="householdHead in householdHeads" :key="householdHead" :value="householdHead">
                {{ householdHead }}
              </option>
            </select>
          </div>
        </div>
        <div v-if="selectedHouseholdHeads.length > 0 && farmers.length > 0" class="total-count">
        <h2>Total Farmers: {{ farmers.length }}</h2>
      </div>
  
        <!-- Display farmers for the selected household heads -->
        <div v-if="farmers.length > 0">
          <h2>Farmers with Household Heads: {{ selectedHouseholdHeads.join(', ') }}:</h2>
        ...
        <div v-else-if="selectedHouseholdHeads.length > 0">
          <p>No farmers found with household head(s): {{ selectedHouseholdHeads.join(', ') }}.</p>
        </div>
      </div>
    </div>
  </template>
  ...
  const selectedHouseholdHead = ref('');
  const householdHeads = ref([]);
  const farmers = ref([]);
  
  const token = localStorage.getItem('auth_token');
  
  // Fetch household head options (radio button values)
  const fetchHouseholdHeads = async () => {
    try {
      const response = await axios.get(
        'http://localhost:8055/items/farmers?fields=household_heads', // Adjust the API endpoint
        {
          headers: {
      ...
      const householdHeadList = response.data.data.map(farmer => farmer.household_heads);
      householdHeads.value = [...new Set(householdHeadList)]; // Remove duplicates
    } catch (error) {
      console.error('Error fetching household heads:', error);
    }
  };
  
  // Add selected household head to the list
  const addHouseholdHead = () => {
    if (selectedHouseholdHead.value && !selectedHouseholdHeads.value.includes(selectedHouseholdHead.value)) {
      selectedHouseholdHeads.value.push(selectedHouseholdHead.value);
      selectedHouseholdHead.value = '';
      fetchFarmersByHouseholdHead();
    }
  };
  
  // Remove a selected household head from the list
  const removeHouseholdHead = (householdHead) => {
    const index = selectedHouseholdHeads.value.indexOf(householdHead);
    if (index > -1) {
      selectedHouseholdHeads.value.splice(index, 1);
      fetchFarmersByHouseholdHead();
    }
  };
  
  // Fetch farmers filtered by household head
  const fetchFarmersByHouseholdHead = async () => {
    farmers.value = []; // Clear previous data
    try {
      if (selectedHouseholdHeads.value.length > 0) {
        for (const householdHead of selectedHouseholdHeads.value) {
          const response = await axios.get(
            `http://localhost:8055/items/farmers?filter[household_heads]=${householdHead}`, // Adjust the query
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const downloadPDF = async () => {
    ...
    for (let i = 0; i < farmers.value.length; i += maxRowsPerPage) {
      ...
      pageContent.innerHTML = `
        <h1 style="color: black;">Farmer Report by Household Head - Page ${(i / maxRowsPerPage) + 1}</h1>
        ...
        <thead class="table-primary" style="background-color: #007bff; color: black;">
          <tr>
            ...
            <th style="padding: 0.75rem; text-align: left; border: 1px solid black; color: white;">Household Heads</th>
          </tr>
        </thead>
        <tbody>
          ${farmersPage.map(farmer => `
            <tr>
              ...
              <td style="padding: 0.75rem; text-align: left; border: 1px solid black; color: black;">${farmer.household_heads}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    const options = {
      ...
      filename: 'farmer_report-household-heads.pdf',
...
.selected-household-heads-container {
  align-items: center;
