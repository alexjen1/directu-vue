<template>
    <div class="report_count-container">
      ...
      <div class="content-box" id="reportContent">
        <h1>Farmer Report Search by Farmers Association</h1>
        <div>
          <div class="input">
            <label for="association">Select Farmers Association(s):</label>
            <div class="selected-associations-container">
              <div class="selected-associations">
                <!-- Display selected associations as tags -->
                <span v-for="association in selectedAssociations" :key="association" class="tag">
                  {{ association }}
                  <button @click="removeAssociation(association)">x</button>
                </span>
              </div>
              <!-- Download Button -->
            ...
            <select v-model="selectedAssociation" @change="addAssociation" required spellcheck="false" class="black-select">
              <option disabled value="">Select Farmers Association</option>
              <option v-for="association in associations" :key="association" :value="association">
                {{ association }}
              </option>
            </select>
          </div>
        </div>
        <div v-if="selectedAssociations.length > 0 && farmers.length > 0" class="total-count">
        <h2>Total Farmers: {{ farmers.length }}</h2>
      </div>
  
        <!-- Display farmers for the selected associations -->
        <div v-if="farmers.length > 0">
          <h2>Farmers in {{ selectedAssociations.join(', ') }}:</h2>
        ...
        <div v-else-if="selectedAssociations.length > 0">
          <p>No farmers found in {{ selectedAssociations.join(', ') }}.</p>
        </div>
      </div>
    </div>
  </template>
  ...
  const fetchAssociations = async () => {
    try {
      const response = await axios.get(
        'http://localhost:8055/items/farmers?fields=member_of_any_farmers_association_cooperative',
        {
          headers: {
      ...
      const associationList = response.data.data.map(farmer => farmer.member_of_any_farmers_association_cooperative);
      associations.value = [...new Set(associationList)];
    } catch (error) {
  ...
  const addAssociation = () => {
    if (selectedAssociation.value && !selectedAssociations.value.includes(selectedAssociation.value)) {
      selectedAssociations.value.push(selectedAssociation.value);
      selectedAssociation.value = '';
      fetchFarmersByAssociation();
    }
  };
  
  const removeAssociation = (association) => {
    const index = selectedAssociations.value.indexOf(association);
    if (index > -1) {
      selectedAssociations.value.splice(index, 1);
      fetchFarmersByAssociation();
    }
  };
  
  const fetchFarmersByAssociation = async () => {
    farmers.value = []; // Clear previous data to reorder based on selection
    try {
      if (selectedAssociations.value.length > 0) {
        for (const association of selectedAssociations.value) {
          const response = await axios.get(
            `http://localhost:8055/items/farmers?filter[member_of_any_farmers_association_cooperative]=${association}`,
            {
              headers: {
                Authorization: `Bearer ${token}`, 
              },
  ...
  const downloadPDF = async () => {
    ...
    for (let i = 0; i < farmers.value.length; i += maxRowsPerPage) {
      ...
      pageContent.innerHTML = `
        <h1 style="color: black;">Farmer Report by Farmers Association - Page ${(i / maxRowsPerPage) + 1}</h1>
        ...
        <thead class="table-primary" style="background-color: #007bff; color: black;">
          <tr>
            ...
            <th style="padding: 0.75rem; text-align: left; border: 1px solid black; color: white;">Farmers Association</th>
          </tr>
        </thead>
        <tbody>
          ${farmersPage.map(farmer => `
            <tr>
              ...
              <td style="padding: 0.75rem; text-align: left; border: 1px solid black; color: black;">${farmer.member_of_any_farmers_association_cooperative}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
    ...
    const options = {
      ...
      filename: 'farmer_report_farmers_association.pdf',
