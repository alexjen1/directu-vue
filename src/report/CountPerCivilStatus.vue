<template>
  <div class="report_count-container">
    ...
    <div class="content-box" id="reportContent">
      ...
      <!-- Display grouped farmers by civil_status -->
...
const groupedFarmers = ref([]); // To hold grouped farmers by civil_status
const token = localStorage.getItem('auth_token');

// Fetch and group farmers by civil_status
const fetchFarmers = async () => {
  try {
    const response = await axios.get(
      'http://localhost:8055/items/farmers?fields=civil_status', // Change `religion` to `civil_status`
      {
        headers: {
    ...
    groupedFarmers.value = Object.entries(civilStatusCounts).map(([civilStatus, count], index) => ({
